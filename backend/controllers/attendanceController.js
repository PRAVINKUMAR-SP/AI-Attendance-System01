...
export const markAttendance = async (req, res) => {
    try {
        ...
        if (user.email) sendEmail(user.email, emailMsg, user.userId, user.name, 'Present').catch(err => console.error("Email fail:", err.message));
        if (user.parentEmail) {
            const pMsg = `Notification: Your ward ${user.name} has arrived at college.\n\nTime: ${time}`;
            sendEmail(user.parentEmail, pMsg, user.userId, user.name, 'Present').catch(err => console.error("Parent Email fail:", err.message));
        }
...
export const processDailyAbsences = async (req, res) => {
    try {
        ...
        for (const student of absentStudents) {
            ...
            if (student.email) sendEmail(student.email, emailMsg, student.userId, student.name, 'Absent').catch(err => console.error("Absence Email fail:", err.message));
            if (student.parentEmail) sendEmail(student.parentEmail, emailMsg, student.userId, student.name, 'Absent').catch(err => console.error("Absence Parent Email fail:", err.message));
